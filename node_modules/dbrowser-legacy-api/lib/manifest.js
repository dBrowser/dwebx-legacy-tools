...
// helper to generate a new dwebx.json object
function generateManifest (manifest = {}) {
  ...
  if (isString(author)) {
    if (author.startsWith('dwebx://') || DAT_HASH_REGEX.test(author)) {
      author = {url: author}
    } else {
      author = {name: author}
    }
  ...
  if (isObject(author)) {
    ...
    if (isString(author.url) && (author.url.startsWith('dwebx://') || DAT_HASH_REGEX.test(author.url))) {
